...
namespace SpriteFontPlus.Samples.TtfBaking.MonoGame.SpriteFontPlus
{
    ...
    {
        public static TtfFontBakerResult Bake(string filepath, float fontPixelHeight,
            int bitmapWidth, int bitmapHeight, IEnumerable<CharacterRange> characterRanges)
        {
            return Bake(File.ReadAllBytes(filepath), fontPixelHeight, bitmapHeight, bitmapHeight, characterRanges);
        }
        ...
        public static unsafe TtfFontBakerResult Bake(byte[] ttf, float fontPixelHeight,
            int bitmapWidth, int bitmapHeight, IEnumerable<CharacterRange> characterRanges)
        {
            if (ttf == null || ttf.Length == 0) throw new ArgumentNullException(nameof(ttf));
